...
public class BaseActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		iv_dvd.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View view) {
				...
				// 第三个参数是传入一个关于这个view信息的任意对象（getLocalState），它即你需要在拖拽监听中的调用event.getLocalState()获取到这个对象来操作用的(比如传入一个RecyclerView中的position)。如果不需要这个对象，传null

				iv_dvd.startDrag(null, new DragShadowBuilder(view), view, 0);
				return true;
			}
		});
		// 设备控制
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			db.execSQL("insert into temp_data  (data)values(?)",
					new String[] { String.valueOf(temp) });
			db.execSQL("insert into hum_data  (data)values(?)",
					new String[] { String.valueOf(temp) });
			db.execSQL("insert into ill_data  (data)values(?)",
					new String[] { String.valueOf(ill) });
			db.execSQL("insert into smo_data  (data)values(?)",
					new String[] { String.valueOf(smo) });
			db.execSQL("insert into gas_data  (data)values(?)",
					new String[] { String.valueOf(gas) });
			db.execSQL("insert into press_data  (data)values(?)",
					new String[] { String.valueOf(press) });
			db.execSQL("insert into pm_data  (data)values(?)",
					new String[] { String.valueOf(pm) });
			db.execSQL("insert into co_data  (data)values(?)",
					new String[] { String.valueOf(co) });
			db.execSQL("insert into per_data  (data)values(?)",
					new String[] { String.valueOf(per) });
