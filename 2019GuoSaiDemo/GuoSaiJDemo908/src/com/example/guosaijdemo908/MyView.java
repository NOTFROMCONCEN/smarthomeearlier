...
public class MyView extends View {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					...
					if (LinkActivity.draw_state) {
						data.add(LinkActivity.getdata);
						Xlable.add(Float.toString(LinkActivity.getdata));
						num++;
					}
	...
	protected void onDraw(Canvas canvas) {
		...
		// 数值大于100写Y轴最高
		if (LinkActivity.getdata > 100) {
			canvas.drawText(Float.toString(LinkActivity.getdata), Xpoint - 30,
