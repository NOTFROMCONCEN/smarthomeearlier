...
public class XgActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnxg=(Button)findViewById(R.id.buttijiao);
		...
		btnxg.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (cursor.getCount()<=0) {
					...
					Cursor cursor1=db.rawQuery("select * from user where pwd=?", arg);
					if (cursor1.getCount()<=0) {
							...
							int i=db.update("user", cValues, "pwd=?", new String[]{etname.getText().toString()});
							if (i>0) {
								AlertDialog.Builder builder=new AlertDialog.Builder(XgActivity.this);
