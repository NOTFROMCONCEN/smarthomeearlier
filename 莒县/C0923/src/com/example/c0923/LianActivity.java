...
public class LianActivity extends Fragment {
	Spinner spws,spdx;
	EditText etlian,etfen;
	Button btnlian;
	Switch swbjd,swlamp,swfan,swdoor;
	TextView tvshijian;
	long fen,miao,shi;
	boolean kg=false;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		etfen=(EditText)view.findViewById(R.id.Edfen);
		btnlian=(Button)view.findViewById(R.id.butlian);
		swbjd=(Switch)view.findViewById(R.id.switch1);
		swfan=(Switch)view.findViewById(R.id.Switch2);
		swlamp=(Switch)view.findViewById(R.id.Switch3);
		swdoor=(Switch)view.findViewById(R.id.Switch4);
		tvshijian=(TextView)view.findViewById(R.id.tejishi);
		...
		btnlian.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (btnlian.getText().toString().equals("开启联动")) {
					...
					if (kg) {
						shi=Integer.parseInt(etfen.getText().toString())*60*1000;
						timer=new CountDownTimer(shi,1000) {
							...
							public void onTick(long millisUntilFinished) {
								...
								tvshijian.setText("联动模式还有"+fen+"分"+miao+"秒");
							}
