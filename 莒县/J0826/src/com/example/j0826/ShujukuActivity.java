...
public class ShujukuActivity extends Activity {
	Spinner spws;
	ListView lv;
	Button btncha;
	List<Map<String, String>> list=new ArrayList<Map<String,String>>();
	public static boolean btnflag=false,jiben=false,login=false;
	Cursor cursor;
	Handler handler;
	Runnable runnable;
...
//		btncha.setEnabled(false);
		try {
			db=this.openOrCreateDatabase("smart1.db", Context.MODE_PRIVATE, null);
		...
		handler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if (btnflag) {
					...
					startActivity(new Intent(ShujukuActivity.this,ZongActivity.class));
					finish();
					jiben=false;
				}else if (login) {
					startActivity(new Intent(ShujukuActivity.this,MainActivity.class));
					finish();
					login=false;
				}
		...
		btncha.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				idc=1;
				SimpleAdapter adapter=new SimpleAdapter(ShujukuActivity.this,list,R.layout.listview,new String[]{"id","ming","zhi"},new int[]{R.id.te1,R.id.te2,R.id.te3});
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.activity_shujuku, menu);
		return true;
	}
