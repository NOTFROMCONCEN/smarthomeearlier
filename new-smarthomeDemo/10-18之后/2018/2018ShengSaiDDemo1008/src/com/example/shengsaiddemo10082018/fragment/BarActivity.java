...
import com.example.shengsaiddemo10082018.LoginActivity;
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (web_state) {
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								if (arg0.equals(ConstantUtil.FAILURE)) {
									System.out.println(LoginActivity.IP
											+ "连接失败" + "Log："
											+ ConstantUtil.FAILURE);
									DiyToast.showToast(getApplicationContext(),
											"连接到：" + LoginActivity.IP + "失败");
								} else {
									System.out.println(LoginActivity.IP
											+ "连接成功");
									DiyToast.showToast(getApplicationContext(),
											"连接到：" + LoginActivity.IP + "成功");
									web_state = false;
								}
							}
						});
				...
				handler.postDelayed(timeRunnable, 4000);
			}
		}
	};
