...
public class MyView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (DrawActivity.getdata > 100) {
			canvas.drawText(Float.toString(DrawActivity.getdata), Xpoint - 40,
					Ypoint - 5 * Yheight, paint);
			canvas.drawLine(Xpoint, Ypoint - 5 * Yheight, Xpoint - 10, Ypoint
					- 5 * Yheight, paint);
		} else {
			canvas.drawText("", Xpoint - 40, Ypoint - 5 * Yheight, paint);
			canvas.drawLine(Xpoint, Ypoint - 5 * Yheight, Xpoint, Ypoint - 5
					* Yheight, paint);
		}
		if (data.size() > 1) {// 如果data大于1 即开始绘图
